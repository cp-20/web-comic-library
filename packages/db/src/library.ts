...
export class PostgresLibrary implements LibraryRepository {
  ...
  async deleteContentReadRecords(
    ...
    await this.#foundation.withSession(context, (session) =>
      session.unsafe(
        "delete from content_read_records where user_id = $1 and resolve_catalog_redirect('content_unit'::catalog_redirect_resource, content_unit_id) = any(select resolve_catalog_redirect('content_unit'::catalog_redirect_resource, id) from unnest($2::uuid[]) as target(id))",
  ...
  async findWorkReadModel(workId: string): Promise<LibraryWorkReadModel | null> {
    const works = await this.#client<{ id: string }[]>`
      select id::text
      from works
      where id = resolve_catalog_redirect('work'::catalog_redirect_resource, ${workId}::uuid)
    ...
    const catchUpRows = await this.#client<{ contentUnitId: string }[]>`
      ...
      where mapping.work_id = ${work.id}::uuid and mapping.confirmed
  ...
  async listReadContentUnitIds(userUuid: string, workId: string): Promise<readonly string[]> {
    const rows = await this.#client<{ contentUnitId: string }[]>`
      select distinct
        resolve_catalog_redirect('content_unit'::catalog_redirect_resource, content_unit_id)::text
          as "contentUnitId"
      from content_read_records
      where user_id = ${userUuid}
        and resolve_catalog_redirect('work'::catalog_redirect_resource, work_id)
          = resolve_catalog_redirect('work'::catalog_redirect_resource, ${workId}::uuid)
      order by "contentUnitId"
    `;
  ...
  async saveLibraryEntry(
    ...
    await this.#foundation.withSession(context, async (session) => {
      await session`
        ...
        values (
          ${entry.userUuid},
          resolve_catalog_redirect('work'::catalog_redirect_resource, ${entry.workId}::uuid),
          ${entry.status}::reading_status,
